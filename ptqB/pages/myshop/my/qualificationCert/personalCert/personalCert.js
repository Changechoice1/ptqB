...
Page(A.assignPage({
    ...
    onLoad(options) {
       ...
       wx.hideShareMenu();
        ...
        var isAuth = options.is_authentication || 0
        that.setData({ AuditF: AuditF, isAuth: isAuth })
        http._post1('StoreAuthentication/auth_info', {
            ...
            if (data.status == 1) {
                var info = data.info
                var imgArr = [info.front, info.verso, info.papers]
